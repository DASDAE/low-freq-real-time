...
def get_edge_effect_time(sampling_interval, total_T, fun = _down_sample_processing, tol = 1e-6, **kargs):
    ...
    new_taxis = (new_taxis - new_taxis[0])/np.timedelta64(1,'s')-N//2*sampling_interval

    edge_t = max(np.abs(new_taxis[ind_1]), np.abs(new_taxis[ind_2]))
    
    if int(np.ceil(edge_t*freq))*2 >= int(total_T*freq):
        raise ValueError(f'edge_t value ({edge_t} sec) is too close to half of the processing chunk size ({total_T} sec).\
                        If your spool contains enough data (at least roughly more than 180 seconds,) please increase memory_size or tolerance.')

    return edge_t

def get_patch_time(memory_size, sampling_rate, num_ch, bytes_per_element=8, processing_factor=5, memory_safety_factor=1.2):
    mem_size_per_second = sampling_rate*num_ch*bytes_per_element*processing_factor*memory_safety_factor/1e6 # in MB
    patch_length = memory_size/mem_size_per_second # in sec
    return patch_length

def waterfall_plot(some_data, min_sec, max_sec, min_ch, max_ch, surface_fiber_in_ft, sample_rate, fig_title, fig_dir, fig_name):
      # Basic error checking
      if (min_sec >= max_sec) or (min_sec < 0) or (max_sec*sample_rate > some_data.shape[1]):
        print("ERROR in plotSpaceTime inputs minSec: "+str(min_sec)+" or maxSec: "+str(max_sec))
        return
      if (min_ch >= max_ch) or (min_ch < 0) or (max_ch > some_data.shape[0]):
        print("Error in plotSpaceTime inputs minCh: "+str(min_ch)+" or maxCh: "+str(max_ch)+" referring to array with "+str(some_data.shape[0])+" channels.")
        return
      ...
      plt.imshow(some_data[min_ch:max_ch,minSecID:maxSecID],aspect='auto',interpolation='none',cmap='seismic',extent=(min_sec,max_sec,(max_ch+ch_start)*channel_spacing*3.28084-surface_fiber_in_ft,(min_ch+ch_start)*channel_spacing*3.28084-surface_fiber_in_ft),vmin=-clip_val,vmax=clip_val)
      plt.ylabel('MD (ft)',fontsize=10)
      plt.xlabel('Time (sec) \n (4 hours, starting from 2023/03/22 03:00:00 UTC)' ,fontsize=10)
...
class LFProc:
    ...
    def _default_process_parameters(self):
        para = {'output_sample_interval': 1.0, # in seconds
                'process_patch_size':100, # in number of output_sample_interval
                'edge_buff_size':10, # in number of output_sample_interval
    ...
    def process_time_range(self,bgtime,edtime):
        ...
        def merge_and_process(DASdata):
            DASdata = self._spool.select(time=(time_grid[data_end-2*buff_size],time_grid[new_data_end])) 
