...
def _get_filename(bgtime, edtime) -> str:
    filename = "LFDAS_" + _get_timestr(bgtime) + "_" + _get_timestr(edtime) + ".h5"
    return filename
...
def get_edge_effect_time(
    ...
    if int(np.ceil(edge_t * freq)) * 2 >= int(total_T * freq):
        raise ValueError(
            f"edge_t value ({edge_t} sec) is too close to half \
            of the processing chunk size ({total_T} sec).\
            If your spool contains enough data (at least roughly more than 180 seconds)\
